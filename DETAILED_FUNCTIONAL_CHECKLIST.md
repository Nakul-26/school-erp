...
| Super-admin: list/create/delete institutions | ⚠️ | ❌ | — | **Backend only, no frontend** | `institutions.routes.ts:25-41,60-82,175-195` has safeguarded delete; no admin UI screen exists |
...
| Teaching-allocations bulk/dashboard/conflicts endpoints | ⚠️ | — | — | Backend-only | No dedicated frontend page consumes these |
...
| Update exam | ✅ | ❌ | — | Needs testing | Backend supports it (`PUT /:id`, lock + override check) but **no edit UI found** in `Exams.tsx` |
...
| **Known bug — `saveMarks` max_marks bind** | ⚠️ | — | — | Confirmed, not UI-reachable | `exams.repository.ts:152` — no `\|\| null` fallback; raw API omitting `max_marks` crashes with a raw D1_TYPE_ERROR (unhandled 500, since the route's catch only handles typed service errors). `Exams.tsx` always supplies a fallback, so not reachable via the app |
...
| Issuance history | ⚠️ | — | — | Backend-only | `GET /issuances/:studentId` exists, not surfaced in `Certificates.tsx` |
