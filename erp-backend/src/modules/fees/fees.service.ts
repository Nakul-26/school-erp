...
export class FeesService {
  ...
  async makePayment(institutionId: string, input: CreatePaymentInput, userId?: string): Promise<{ paymentId: string; receiptId: string; receiptNumber: string }> {
    ...
    // The guard update and the payment/receipt/ledger inserts all run as ONE
    // atomic D1 batch (see buildPaymentBatchStatements) - every insert is
    // conditioned on the guard having actually applied, so a request that
    // dies mid-flight can't leave a paid_amount bump with no payment record
    // behind it (found via load testing: two separate calls here used to
    // leave exactly that gap under concurrent load).
    const [updateResult] = await this.repo.runBatch(
      this.repo.buildPaymentBatchStatements({
        recordId: input.student_fee_record_id,
        expectedPaidAmount: record.paid_amount,
        newPaidAmount,
        status,
        institutionId,
        input,
