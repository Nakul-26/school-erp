...
| 3 | Institution & academic setup | ✅ | 🟡 | ⚠️ Partial | Academic Calendar permission gating (2026-08-28) and required-field validation (2026-08-30) both fixed & live-verified; still no frontend for super-admin institution list/create |
...
| 9 | Exams, grading, report cards | ✅ | 🟡 | 🟡 Needs Testing | Live-tested full flow this session incl. out-of-range rejection (works); bulk report-card generation UI added & live-verified 2026-08-28 (report cards still browser-print, not real PDFs) |
...
| 24 | Certificates | ✅ | ❌ | 🟡 Needs Testing | Real data substitution confirmed; output is browser-print HTML, not a generated PDF file |
...
- Super-admin institution list/create/delete — `InstitutionSetup.tsx` only edits the caller's own institution.
...
- Certificate issuance history (`GET /certificates/issuances/:studentId`) — not surfaced anywhere.
- Teaching-allocations bulk-allocation/dashboard/conflicts endpoints — no dedicated frontend page consumes them.
